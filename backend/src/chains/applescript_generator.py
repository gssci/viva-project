...
def verify_applescript_syntax(script_content: str):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            error_msg = (
                result.stderr.strip() or "Unknown syntax error during compilation."
            )
        ...
        print(
            "Warning: 'osacompile' utility not found. Skipping syntax verification (requires macOS)."
        )
...
def generate_and_verify_applescript(user_query: str, max_retries: int = 2) -> str:
    ...
    base_prompt = ChatPromptTemplate.from_messages(
        [("system", system_instruction), ("user", "{input}")]
    )
